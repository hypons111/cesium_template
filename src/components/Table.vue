<template>
  <div id="report">
    <div id="reportComponent">
      <aside>
        <nav>
          <a :class="{ currentReport: currentReport === 'a' }" @click="switchReport('a')">Normal Table</a>
          <a :class="{ currentReport: currentReport === 'b' }" @click="switchReport('b')">Table b</a>
          <a :class="{ currentReport: currentReport === 'c' }" @click="switchReport('c')">Table c</a>
          <a :class="{ currentReport: currentReport === 'd' }" @click="switchReport('d')">Table d</a>
        </nav>
      </aside>
      <main>

        <el-table id="report_a" :data="tableData.data" v-if="currentReport === 'a'">
          <el-table-column 
        ...
        <el-table id="report_c" :data="tableData.data" v-if="currentReport === 'c'">
          <el-table-column 
        ...
        <el-table id="report_d" :data="tableData.data" v-if="currentReport === 'd'">
...
function test() {
  console.log(allTableData.value.data);
  const a = allTableData.value.data[0]
  tableData.value.data = [];
  tableData.value.data.push(a);
  ...
  #reportComponent {
    ...
    main {
      ...
      .el-table {
        height: 100%;
        background-color: rgba(var(--BLACK), 0.5);
        --el-table-border-color: transparent; // 重設 element-plus 設定
        border: 5px solid rgb(var(--YELLOW));
        color: rgb(var(--WHITE));
        ...
        ::v-deep {
          thead tr {
            height: 4rem;
            font-size: 1.1rem;
            --el-table-header-bg-color: rgb(var(--R7));
            --el-table-border: 0.25rem solid rgb(var(--R3));
          }
          tbody tr {
            font-size: 1rem;
            --el-table-tr-bg-color: rgb(var(--B7));
            --el-table-border: 0.25rem solid rgb(var(--B3));
            --el-table-row-hover-bg-color: rgb(var(--Y7));
          }
          tbody tr:nth-child(even) {
            background-color: rgb(var(--G7));
          }
        }
      }
    }
